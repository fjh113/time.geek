...
public class Example02 {
    ...
    public static void main(String[] args) {
        ...
        //循环检查每个数组元素的值，在给数组元素赋值前，数组元素的值其实都是每种类型的初始值
        for (int i = 0;i<multiDimensionArr.length;i++){
            for (int j=0;j<multiDimensionArr[i].length;j++){
                System.out.println("multiDimensionArr["+i+"]="+multiDimensionArr[i][j]);
            }
        }
        //其实第一堆的数组，每个元素都是下一维度的属于的变量，在这里我们可以改变这个值
        // 也就是让它指向一个新的变量。
        multiDimensionArr[2]=new double[100];
        //检查每个数组的长度，发现最后一个的长度不一样了，因为它指向了新的数组
        for (int i=0;i<multiDimensionArr.length;i++){
